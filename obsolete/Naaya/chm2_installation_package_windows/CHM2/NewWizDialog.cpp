...
void CNewWizDialog::Init()
{
  m_nPlaceholderID = 0;
	...
	// use this flag to know when to hide some buttons
	// for the single error page don't show back, next and cancel
	m_bErrorPage = FALSE;
}
...
void CNewWizDialog::OnCancel()
{
	...
	{
		if (IDOK == MessageBox("Do you want to abort this installation?", CRString(IDS_ABORT_WIZARD_CAPTION), MB_OKCANCEL | MB_ICONQUESTION))
			CDialog::OnCancel();
	}
}