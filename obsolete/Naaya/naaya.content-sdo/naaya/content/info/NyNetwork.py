...
#module constants
INFO_TYPE = skel.INFO_TYPES['networks']
METATYPE_OBJECT = INFO_TYPE['meta_type']
LABEL_OBJECT = INFO_TYPE['meta_label']
PREFIX_OBJECT = INFO_TYPE['prefix']
OBJECT_FORMS = []
OBJECT_CONSTRUCTORS = ['sdo_info_add', 'addNyNetwork']
OBJECT_ADD_FORM = 'network_add_html'
DESCRIPTION_OBJECT = 'This is Naaya Network type.'

DEFAULT_SCHEMA = deepcopy(DEFAULT_SCHEMA)

#Define folder categories
DEFAULT_SCHEMA['sdo_network_type'] = dict(sortorder=12, widget_type='Select',
                label='Network type', list_id='sdo_network_type', property_type='Sdo category')
DEFAULT_SCHEMA['sdo_scope'] = dict(sortorder=13, widget_type='Select',
                label='Scope', list_id='sdo_scope', property_type='Sdo category')
DEFAULT_SCHEMA['sdo_audience'] = dict(sortorder=14, widget_type='SelectMultiple',
                label='Audience', list_id='sdo_audience', property_type='Sdo category')
DEFAULT_SCHEMA['sdo_status'] = dict(sortorder=15, widget_type='Select',
                label='Status', list_id='sdo_status', property_type='Sdo category')
...
def addNyNetwork(self, id='', REQUEST=None, contributor=None, **kwargs):
    ...
    if REQUEST is None:
        ...
        for k, v in DEFAULT_SCHEMA.items():
            if 'property_type' in DEFAULT_SCHEMA[k] and\
...
InitializeClass(NyNetwork)

config.update({
    'constructors': (network_add_html, addNyNetwork),
    'folder_constructors': [
            ('network_add_html', network_add_html),
            ('addNyNetwork', addNyNetwork),
        ],
    'add_method': addNyNetwork,
    'validation': issubclass(NyNetwork, NyValidation),
    '_class': NyNetwork,
})
