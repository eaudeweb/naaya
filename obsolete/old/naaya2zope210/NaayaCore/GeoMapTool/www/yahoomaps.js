...
YGeoMapTool.prototype.createMarker = function(lat, lng, html, title, icon) {
	...
	if (title === 'cluster') {
		YEvent.Capture(marker, EventsList.MouseClick, function() {
			map.drawZoomAndCenter(point, map.getZoomLevel() - 1);
		});
	} else {
		YEvent.Capture(marker, EventsList.MouseClick, function() {
...
function getMarkerData(marker) {
	var latitude = marker.YGeoPoint.Lat;
 ...
 */
YGeoMapTool.prototype.createIconSymbol = function(imageURL, width, height) {
	icon = new YImage();
	icon.src = imageURL;
...
YGeoMapTool.prototype.showMapLocations = function(){
	YEvent.Capture(map, 'endMapDraw', startMapRefresh);
}
/**
...
YGeoMapTool.prototype.getBounds = function () {
	var bounds = map.getBoundsLatLon();
	return {'lat_min': bounds.LatMin,
