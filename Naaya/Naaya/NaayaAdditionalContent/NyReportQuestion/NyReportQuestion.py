...
#module constants
METATYPE_OBJECT = 'Naaya Report Question'
LABEL_OBJECT = 'Report Question'
PERMISSION_ADD_OBJECT = 'Naaya - Add Naaya Report Question objects'
OBJECT_FORMS = ['reportquestion_add', 'reportquestion_edit', 'reportquestion_index', 'reportquestions_index']
OBJECT_CONSTRUCTORS = ['manage_addNyReportQuestion_html', 'reportquestion_add_html', 'addNyReportQuestion', 'importNyReportQuestion']
OBJECT_ADD_FORM = 'reportquestion_add_html'
DESCRIPTION_OBJECT = 'This is Naaya Report Question type.'
PREFIX_OBJECT = 'qtn'
PROPERTIES_OBJECT = {
    'id':                           (0, '', ''),
    'title':                        (1, MUST_BE_NONEMPTY, 'The Question field must have a value.'),
    'description':                  (0, '', ''),
    'coverage':                     (0, '', ''),
    'keywords':                     (0, '', ''),
...
def reportquestion_add_html(self, REQUEST=None, RESPONSE=None):
    """ """
    return self.getFormsTool().getContent({'here': self, 'kind': METATYPE_OBJECT, 'action': 'addNyReportQuestion'}, 'reportquestion_add')

def addNyReportQuestion(self, id='', title='', description='', coverage='', keywords='',
    sortorder='', contributor=None, releasedate='', discussion='', lang=None, REQUEST=None, **kwargs):
    """
    Create a Report Question type of object.
    """
    #process parameters
    ...
    if not len(r):
        ...
        #create object
        ob = NyReportQuestion(id, title, description, coverage, keywords, sortorder, contributor, releasedate, lang)
        ...
        if REQUEST is not None:
            ...
            REQUEST.RESPONSE.redirect('%s/reportquestion_add_html' % self.absolute_url())
        else:
            raise Exception, '%s' % ', '.join(r)

def importNyReportQuestion(self, param, id, attrs, content, properties, discussion, objects):
    pass
    #TODO: to be added
...
#                try: self.manage_delObjects([id])
#                except: pass
#            addNyReportQuestion(self, id=id,
...
class NyReportQuestion(NyAttributes, reportquestion_item, NyContainer, NyCheckControl):
    ...
    icon = 'misc_/NaayaContent/NyReportQuestion.gif'
    icon_marked = 'misc_/NaayaContent/NyReportQuestion_marked.gif'

    def manage_options(self):
        """ """
        l_options = (NyContainer.manage_options[0],) + reportquestion_item.manage_options
    ...
    def __init__(self, id, title, description, coverage, keywords, sortorder, contributor, releasedate, lang):
        """ """
        self.id = id
        reportquestion_item.__dict__['__init__'](self,title, description, coverage, keywords, sortorder, releasedate, lang)
    ...
    def startVersion(self, REQUEST=None):
        ...
        self.version = reportquestion_item(self.title, self.description, self.coverage,
    ...
    manage_edit_html = PageTemplateFile('zpt/reportquestion_manage_edit', globals())

    #site pages
    security.declareProtected(PERMISSION_ADD_OBJECT, 'add_html')
    def add_html(self, REQUEST=None, RESPONSE=None):
        """ """
        return self.getFormsTool().getContent({'here': self}, 'reportquestion_add')

    security.declareProtected(view, 'index_html')
    def index_html(self, REQUEST=None, RESPONSE=None):
        """ """
        return self.getFormsTool().getContent({'here': self}, 'reportquestion_index')

    security.declareProtected(PERMISSION_EDIT_OBJECTS, 'edit_html')
    def edit_html(self, REQUEST=None, RESPONSE=None):
        """ """
        return self.getFormsTool().getContent({'here': self}, 'reportquestion_edit')
