...
class SyndicationTool(Folder, utils, namespaces_tool, channeltypes_manager):
    ...
    def syndicateSomething(self, p_url, p_items=[]):
        ...
        l_rdf.append('<title>%s</title>' % l_site.utXmlEncode(l_site.title))
        l_rdf.append('<link>%s</link>' % p_url)
        l_rdf.append('<description>%s</description>' % self.utXmlEncode(l_site.description))
        ...
        l_rdf.append('<dc:publisher>%s</dc:publisher>' % self.utXmlEncode(l_site.publisher))
        l_rdf.append('<dc:creator>%s</dc:creator>' % self.utXmlEncode(l_site.creator))
        l_rdf.append('<dc:subject>%s</dc:subject>' % self.utXmlEncode(l_site.title))
        l_rdf.append('<dc:subject>%s</dc:subject>' % self.utXmlEncode(l_site.site_subtitle))
        l_rdf.append('<dc:language>%s</dc:language>' % self.utXmlEncode(lang))
        l_rdf.append('<dc:rights>%s</dc:rights>' % self.utXmlEncode(l_site.rights))
        l_rdf.append('<dc:source>%s</dc:source>' % self.utXmlEncode(l_site.publisher))
        l_rdf.append('<items>')
        ...
        if self.hasImage():
            l_rdf.append('<image>')
            l_rdf.append('<title>%s</title>' % self.utXmlEncode(l_site.title))
