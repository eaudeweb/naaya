...
class LocalChannel(SimpleItem, utils):
    ...
    def syndicateThis(self, lang=None):
        ...
        r = []
        r.append('<item rdf:about="%s">' % self.absolute_url())
        r.append('<link>%s</link>' % self.absolute_url())
        r.append('<dc:title>%s</dc:title>' % self.utXmlEncode(self.title_or_id()))
        r.append('<dc:identifier>%s</dc:identifier>' % self.absolute_url())
        r.append('<dc:description>%s</dc:description>' % self.utXmlEncode(self.description))
        r.append('<dc:contributor>%s</dc:contributor>' % self.utXmlEncode(self.contributor))
        r.append('<dc:language>%s</dc:language>' % self.utXmlEncode(self.language))
        r.append('<dc:creator>%s</dc:creator>' % self.utXmlEncode(self.creator))
        r.append('<dc:publisher>%s</dc:publisher>' % self.utXmlEncode(self.publisher))
        r.append('<dc:rights>%s</dc:rights>' % self.utXmlEncode(self.rights))
        r.append('<dc:type>%s</dc:type>' % self.utXmlEncode(self.get_channeltype_title(self.type)))
        r.append('<dc:format>text/xml</dc:format>')
        r.append('<dc:source>%s</dc:source>' % self.utXmlEncode(self.publisher))
        r.append('</item>')
        return ''.join(r)
    ...
    def index_html(self, REQUEST=None, RESPONSE=None):
        ...
        r.append('<title>%s</title>' % self.utXmlEncode(self.title))
        r.append('<link>%s</link>' % l_site.absolute_url())
        r.append('<description>%s</description>' % self.utXmlEncode(self.description))
        r.append('<dc:identifier>%s</dc:identifier>' % l_site.absolute_url())
        ...
        r.append('<dc:subject>%s</dc:subject>' % self.utXmlEncode(l_site.getLocalProperty('site_title', lang)))
        r.append('<dc:subject>%s</dc:subject>' % self.utXmlEncode(l_site.getLocalProperty('site_subtitle', lang)))
        ...
        r.append('<dc:type>%s</dc:type>' % self.utXmlEncode(self.type))
