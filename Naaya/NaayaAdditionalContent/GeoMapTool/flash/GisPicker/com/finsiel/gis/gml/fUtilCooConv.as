...
class com.finsiel.gis.gml.fUtilCooConv {

	//in radioans
	// = {Lat:Latitude_output, Long:Longitude_output};
	function fUtilCooConv() {
		
	}
	public function convert(_Easting:Number, _Northing:Number, _type:String, _format:String, _decimals:Number):Object {
		...
		if (type == "DD") {
			if (format == "GEOG") {
				if (Latitude_display<=0) {
					Latitude_output = Math.abs(Latitude_display)+" S";
				}
				if (Latitude_display>0) {
					Latitude_output = Math.abs(Latitude_display)+" N";
				}
				if (Longitude_display<=0) {
					Longitude_output = Math.abs(Longitude_display)+" W";
				}
				if (Longitude_display>0) {
					Longitude_output = Math.abs(Longitude_display)+" E";
				}
			} else {
				Latitude_output = Latitude_display.toString();
				Longitude_output = Longitude_display.toString();
			}
		} else {
			if (format == "GEOG") {
				if (Latitude_display<=0) {
				...
				if (Latitude_display<=0) {
					Latitude_output = "-"+conv(Math.abs(Latitude_display));
				}
				if (Latitude_display>0) {
					Latitude_output = conv(Math.abs(Latitude_display));
				}
				if (Longitude_display<=0) {
					Longitude_output = "-"+conv(Math.abs(Longitude_display));
				}
				if (Longitude_display>0) {
					Longitude_output = conv(Math.abs(Longitude_display));
				}
	...
	function conv(val:Number):String {
		...
		var conv_str = gr.toString()+"°"+min_disp+"'"+sec_disp+"'"+"'";
		return (conv_str);
	}
}
