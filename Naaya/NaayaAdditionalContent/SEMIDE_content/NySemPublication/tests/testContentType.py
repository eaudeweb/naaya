...
from Products.NaayaContent.NySemPublication.NySemPublication import addNySemPublication
...
class NaayaContentTestCase(NaayaTestCase.NaayaTestCase):
    ...
    def test_main(self):
        """ Add, Find, Edit and Delete Naaya Semide Publication """
        addNySemPublication(self._portal().info, id='doc1', title='doc1', lang='en', submitted=1)
        addNySemPublication(self._portal().info, id='doc1_fr', title='doc1_fr', lang='fr', submitted=1)
        
        docs = self._portal().getCatalogedObjectsCheckView(meta_type=['Naaya Semide Publication'])
        ...
        brains = self._portal().getCatalogedObjectsCheckView(meta_type=['Naaya Semide Publication'])
