...
from Products.NaayaForum.constants                  import *
from Products.SEMIDEPhotoArchive.constants          import *
...
#        flash_tool.df_template = 'monthly'
    ...
    def get_containers_metatypes(self):
        ...
        return [METATYPE_FOLDER, 'Folder', METATYPE_NYPHOTOFOLDER, METATYPE_NYSEMTHEMATICDIR]
    ...
    def getResourceListing(self, query='', meta_types=[], textlaws_props=[], document_props=[],
        ...
        meta_types =        utConvertToListExact(meta_types)
        languages =         utConvertToListExact(languages)
        textlaws_props =    utConvertToListExact(textlaws_props)
        document_props =    utConvertToListExact(document_props)
        multimedia_props =  utConvertToListExact(multimedia_props)
    ...
    def getNavFolders(self, l_folder):
        ...
        l_buff = self.utSortObjsListByAttr([x for x in l_folder.objectValues([METATYPE_FOLDER, METATYPE_NYPHOTOFOLDER, METATYPE_NYSEMTHEMATICDIR]) if x.submitted==1], "title", 0)
    ...
    def _getLocation(self, p_location):
        ...
        if self._testIsRoot(p_location):
            ...
            if p_location.meta_type not in [METATYPE_FOLDER]:
        ...
        for l_folder in l_folders:
            if ((len(l_folder.objectValues(self.get_semide_containers_metatypes())) > 0) or ((len(l_folder.getObjects()) > 0) and showitems==1) or (l_folder.id=='about' and self is root)):
                if l_folder.absolute_url(1) in expand or 'all' in expand:
                    l_tree.append((l_folder, 0, depth))
                    if showitems:
                        buf = l_folder.getPublishedObjects()
                        for l_item in buf:
                            l_tree.append((l_item, -1, depth+1))
    ...
    def getHighlightWordsInText(self, p_text, p_words="", p_highlight_start="<span class='hlighted'>",
        ...
        return html_utils().highlightWordsInText(p_text, p_words, p_highlight_start, p_highlight_end, p_phrases, p_nosplit)

    def getHighlightWordsInHtml(self, p_text, p_words="", p_highlight_start="<span class='hlighted'>",
                                p_highlight_end="</span>", p_phrases = 3, p_nosplit = False):
        return html_utils().highlightWordsInHtml(p_text, p_words, p_highlight_start, p_highlight_end, p_phrases, p_nosplit)
    ...
    def externalSearch(self, servers=[], query='', sort_expr='', order='', page_search_start='', search_languages=[]):
        """ """
        list_results = []
        try: page_search_start = int(page_search_start)
        except: page_search_start = 0
        if query.strip() != '':
            ...
            results = self.external_ew_search(servers, query, search_languages)
            if len(results) > 1:
    ...
    def external_ew_search(self, servers, query, search_languages):
        results = []
        ...
        for server in servers:
            ...
            try:
                ...
                results.append(site.external_search(query, search_languages))
            except:
    ...
    def external_search(self, search_param='', search_languages=[]):
        """ """
        ...
        results = []
        search_param = self.translateQuery(search_param, search_languages)
        for lang in search_languages:
            l_expr = "self.searchCatalog(search_param[\'%s\'], '', '%s')" % (lang, lang)
            results.extend(eval(l_expr))
    ...
    def stripAllHtmlTags(self, p_text):
        """ """
        return html_utils().stripAllHtmlTags(p_text)
