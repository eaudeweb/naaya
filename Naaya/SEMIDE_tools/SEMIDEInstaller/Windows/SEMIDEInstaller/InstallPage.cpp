...
BOOL CInstallPage::CopyNecessaryFiles(CStringArray& arrLog)
{
	...
	// move EMWISUninstall.exe from bin to application path
	CString strFilePath, strBuffer;
	strFilePath.Format("%s\\EMWISUninstall.exe", pWizardData->m_strPath + strBinFolder);
	strBuffer.Format(CRString(IDS_MOVE_FILE), strFilePath, pWizardData->m_strPath + "\\EMWISUninstall.exe");
    arrLog.Add(strBuffer);
	if (toolz.InternalOverwriteFile(pWizardData->m_strPath + strBinFolder + "\\EMWISUninstall.exe", pWizardData->m_strPath + "\\EMWISUninstall.exe"))
	{
		strError.Format(CRString(IDS_FAILED_MOVE_FILE), pWizardData->m_strPath + strBinFolder + "\\EMWISUninstall.exe", pWizardData->m_strPath + "\\EMWISUninstall.exe");
		arrLog.Add(strError);
		return FALSE;
	}
	else
		toolz.InternalDeleteFile(pWizardData->m_strPath + strBinFolder + "\\EMWISUninstall.exe");

	char path[MAX_PATH];
	SHGetSpecialFolderPath(NULL, path, 0x0025, FALSE);

	// copy libexslt.dll from bin to System32
	strFilePath.Format("%s\\libexslt.dll", path);
	strBuffer.Format(CRString(IDS_COPY_FILE), strFilePath);
    arrLog.Add(strBuffer);
	if (toolz.InternalCopyFile(pWizardData->m_strPath + strBinFolder + "\\libexslt.dll", strFilePath))
	{
		strError.Format(CRString(IDS_FAILED_COPY_FILE), pWizardData->m_strPath + strBinFolder + "\\libexslt.dll");
		arrLog.Add(strError);
		//return FALSE;
	}

	// copy libxml2.dll from bin to System32
	strFilePath.Format("%s\\libxml2.dll", path);
	strBuffer.Format(CRString(IDS_COPY_FILE), strFilePath);
    arrLog.Add(strBuffer);
	if (toolz.InternalCopyFile(pWizardData->m_strPath + strBinFolder + "\\libxml2.dll", strFilePath))
	{
		strError.Format(CRString(IDS_FAILED_COPY_FILE), pWizardData->m_strPath + strBinFolder + "\\libxml2.dll");
		arrLog.Add(strError);
		//return FALSE;
	}

	// copy libxslt.dll from bin to System32
	strFilePath.Format("%s\\libxslt.dll", path);
	strBuffer.Format(CRString(IDS_COPY_FILE), strFilePath);
    arrLog.Add(strBuffer);
	if (toolz.InternalCopyFile(pWizardData->m_strPath + strBinFolder + "\\libxslt.dll", strFilePath))
	{
		strError.Format(CRString(IDS_FAILED_COPY_FILE), pWizardData->m_strPath + strBinFolder + "\\libxslt.dll");
		arrLog.Add(strError);
		//return FALSE;
	}

	// copy zlib1.dll from bin to System32
	strFilePath.Format("%s\\zlib1.dll", path);
	strBuffer.Format(CRString(IDS_COPY_FILE), strFilePath);
    arrLog.Add(strBuffer);
	if (toolz.InternalCopyFile(pWizardData->m_strPath + strBinFolder + "\\zlib1.dll", strFilePath))
	{
		strError.Format(CRString(IDS_FAILED_COPY_FILE), pWizardData->m_strPath + strBinFolder + "\\zlib1.dll");
		arrLog.Add(strError);
		//return FALSE;
	}
...
BOOL CInstallPage::ModifyConfigurationFiles(CStringArray& arrLog)
{
	...
	// modify bin's emwisinit.py
	strFilePath = pWizardData->m_strPath;
	strFilePath += strBinFolder;
	strFilePath += "\\emwisinit.py";
    strBuffer.Format(CRString(IDS_CONFIGURE_FILE), strFilePath);
    arrLog.Add(strBuffer);
	if (toolz.ReadFileContent(strFilePath, strContent, arrLog))
    {	// have to replace the following values:
	...
	// modify bin's emwisinit.bat
	strFilePath = pWizardData->m_strPath;
	strFilePath += strBinFolder;
	strFilePath += "\\emwisinit.bat";
    strBuffer.Format(CRString(IDS_CONFIGURE_FILE), strFilePath);
    arrLog.Add(strBuffer);
	if (toolz.ReadFileContent(strFilePath, strContent, arrLog))
    {	// have to replace the following values:
...
BOOL CInstallPage::CleanUp(CStringArray& arrLog)
{
	...
	// remove bin's libexslt.dll
	strFilePath = pWizardData->m_strPath;
	strFilePath += strBinFolder;
	strFilePath += "\\libexslt.dll";
    strBuffer.Format(CRString(IDS_DELETE_FILE), strFilePath);
	arrLog.Add(strBuffer);
	if (toolz.InternalDeleteFile(strFilePath))
	{
	...
	// remove bin's libxml2.dll
	strFilePath = pWizardData->m_strPath;
	strFilePath += strBinFolder;
	strFilePath += "\\libxml2.dll";
    strBuffer.Format(CRString(IDS_DELETE_FILE), strFilePath);
	arrLog.Add(strBuffer);
	if (toolz.InternalDeleteFile(strFilePath))
	{
	...
	// remove bin's libxslt.dll
	strFilePath = pWizardData->m_strPath;
	strFilePath += strBinFolder;
	strFilePath += "\\libxslt.dll";
    strBuffer.Format(CRString(IDS_DELETE_FILE), strFilePath);
	arrLog.Add(strBuffer);
	if (toolz.InternalDeleteFile(strFilePath))
	{
	...
	// remove bin's zlib1.dll
	strFilePath = pWizardData->m_strPath;
	strFilePath += strBinFolder;
	strFilePath += "\\zlib1.dll";
    strBuffer.Format(CRString(IDS_DELETE_FILE), strFilePath);
	arrLog.Add(strBuffer);
	if (toolz.InternalDeleteFile(strFilePath))
	{
...
BOOL CInstallPage::CreateShortcuts(CStringArray& arrLog)
{
	...
	// create shortcut to EMWIS
	strLinkAddress.Format("http://%s:%d/portal/index_html", pWizardData->m_strHostIPAddress, pWizardData->m_nZopeHTTPPort);
	...
	// create shortcut to EMWIS console
	strBuffer.Format("%s\\EMWISConsole.exe", pWizardData->m_strPath + strBinFolder);
	...
	// create shortcut to EMWIS uninstaller
	strBuffer.Format("%s\\EMWISUninstall.exe", pWizardData->m_strPath);
