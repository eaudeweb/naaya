...
void CTabZope::OnOK() 
{
	((CCHM2ConsoleDlg*)GetParent())->m_pServiceManager->SMStartService(CRString(IDS_SERVICE_NAME_ZOPE));
}

void CTabZope::OnCancel() 
{
	((CCHM2ConsoleDlg*)GetParent())->m_pServiceManager->SMStopService(CRString(IDS_SERVICE_NAME_ZOPE));
}
...
{
	CServiceManager* pServiceManager = ((CCHM2ConsoleDlg*)GetParent())->m_pServiceManager;
	CString strError;
