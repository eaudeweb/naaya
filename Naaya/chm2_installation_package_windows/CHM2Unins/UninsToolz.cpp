// UninsToolz.cpp: implementation of the CUninsToolz class.
//
//////////////////////////////////////////////////////////////////////
...
BOOL CUninsToolz::RunningAsAdministrator()
{
	BOOL   fAdmin;
    HANDLE  hThread;
...
BOOL CUninsToolz::UTReadRegistryKey(CString strRegString, CString strKeyName, CString& strKeyValue)
{
   HKEY hKey;
   char strSn[1024] = "";
   DWORD dwBufLen = 1024;
...
BOOL CUninsToolz::UTDeleteRegistryKey(CString strKeyName, CString& strError)
{
...
unsigned long CUninsToolz::internalGetFolderSize(CString status)
{
...
int CUninsToolz::RecDeleteFolder(CString strFolder)
{
	WIN32_FIND_DATA fd;
    HANDLE hp;
	int ret=0;
...
BOOL CUninsToolz::TakeOwnership(CString strFile)
{
	int file[256];
	char error[256];
	DWORD description;
...
BOOL CUninsToolz::SetPrivilege(HANDLE hToken,LPCTSTR lpszPrivilege,BOOL bChange)
{
	TOKEN_PRIVILEGES tp;
	LUID luid;
...
BOOL CUninsToolz::SetPermission(LPCTSTR lpszFile, LPCTSTR lpszAccess, DWORD dwAccessMask)
{
	int buff[512];
	char domain[512];
	char error[256];
...
void CUninsToolz::UpdateProgressBar(unsigned long nSize)
{
