...
class Extra_for_DateRangeIndex:
    """hack for a bug in DateRangeIndex
       'dict' object has no attribute 'since_field' """
...
class CHMSite(NySite):
    ...
    def add_glossary_coverage(self):
        ...
        coverage.saveProperties(
            glossary_id=self.coverage_glossary, all_content_types=True,
            sortorder=coverage.sortorder, title=coverage.title,
    ...
    def add_glossary_keywords(self):
        ...
        self._getOb(ID_GLOSSARY_KEYWORDS).xliff_import(
            self.futRead(join(CHM2_PRODUCT_PATH, 'skel', 'others',
                              'glossary_keywords.xml')))
        self.keywords_glossary = ID_GLOSSARY_KEYWORDS
        schema_tool = self.portal_schemas
        keywords = schema_tool.NyDocument['keywords-property']
        keywords.saveProperties(
            glossary_id=self.keywords_glossary, all_content_types=True,
            sortorder=keywords.sortorder, title=keywords.title,
            visible=keywords.visible)

    security.declareProtected(view_management_screens,
                              'add_chm_terms_glossary')
    ...
    def getNewsArchive(self):
        return self._getOb('news', None)

    def getEventsArchive(self):
        return self._getOb('events', None)
    ...
    def gl_add_site_language_custom(self, language):
        ...
        for b in self.getCatalogedBrains(meta_type=[METATYPE_NYPHOTOFOLDER,
                                                    METATYPE_NYPHOTO]):
    ...
    def gl_del_site_languages_custom(self, languages):
        ...
        for b in self.getCatalogedBrains(meta_type=[METATYPE_NYPHOTOFOLDER,
            ...
            for language in languages:
                try:
                    x.del_language(language)
                except:
                    pass
        for r in self.objectValues(METATYPE_NYNETREPOSITORY):
            for language in languages:
                try:
                    r.del_language(language)
                except:
                    pass
        for gloss in self.objectValues(NAAYAGLOSSARY_CENTRE_METATYPE):
            for language in languages:
    ...
    def gl_change_site_defaultlang_custom(self, language):
        ...
        for b in self.getCatalogedBrains(meta_type=[METATYPE_NYPHOTOFOLDER,
            ...
            x = catalog_tool.getobject(b.data_record_id_)
            try:
                x.manage_changeDefaultLang(language)
            except:
                pass
        for r in self.objectValues(METATYPE_NYNETREPOSITORY):
            try:
                r.manage_changeDefaultLang(language)
            except:
                pass
    ...
    def getOnFrontPhotos(self):
        ...
        r = [x for x in self.getPhotoArchive().getObjects()
             if x.approved == 1 and getattr(x, 'topitem', 0)
             and x.submitted == 1]
        r.sort(lambda x, y: cmp(y.releasedate, x.releasedate)
               or cmp(x.sortorder, y.sortorder))
        return r

    def getUrlMap(self, sort='title'):
        # Processes and returns a map with all approved urls in the portal by
        # domain. This requires NyURL pluggable content type to be present
        urls = self.query_objects_ex(meta_type=METATYPE_NYURL, approved=1)
        other_urls = self.query_objects_ex(
            meta_type=self.get_content_urls().keys(), approved=1)
        if sort == 'url':
            # sort urls
    ...
    def getURLProperties(self):
        # process the list of all approved items which have URL properties,
        # by location
        ...
        for x in self.query_objects_ex(meta_type=METATYPE_NYURL):
            p_value = x.locator
            if isinstance(p_value, unicode):
                p_value = p_value.encode('utf-8')
            if p_value != '':
                if p_value in url_struct:
                    url_struct[p_value].append((x, x.getParentNode()))
                else:
                    url_struct[p_value] = [(x, x.getParentNode())]
        # list other
        for x in self.query_objects_ex(
                meta_type=self.get_content_urls().keys()):
            for y in self.get_content_urls()[x.meta_type]:
                p_value = getattr(x, y)
                if isinstance(p_value, unicode):
                    p_value = p_value.encode('utf-8')
    ...
    def processCreateAccountForm(self, username='', password='', confirm='',
                                 firstname='', lastname='', email='',
                                 REQUEST=None):
        """ Creates an account on the local acl_users and sends an email to
            the maintainer with the account infomation
        """
        ...
        if err:
            if REQUEST:
                ...
                self.setCreateAccountSession(username, firstname, lastname,
                                             email, password)
                return REQUEST.RESPONSE.redirect(REQUEST.HTTP_REFERER)
        if not err:
            self.sendCreateAccountEmail('', firstname + ' ' + lastname, email,
                                        '', username, '', '', '')
        if REQUEST:
    ...
    def sendCreateAccountEmail(self, p_to, p_name, p_email, p_organisation,
                               p_username, p_location_path, p_location_title,
                               p_comments, **kwargs):
    ...
    def duplicate_translate(self, property, source_lang, target_lang):
        ...
        for b in self.getCatalogedBrains():
            ...
            if len(v) <= 0:
                x._setLocalPropValue(property, target_lang,
                                     x.getLocalProperty(property, source_lang))
                x._p_changed = 1
    ...
    security.declareProtected(PERMISSION_ADMINISTRATE,
                              'admin_submissions_html')

    def admin_submissions_html(self, REQUEST=None, RESPONSE=None):
        """ """
        return self.getFormsTool().getContent({'here': self},
                                              'site_admin_submissions')

    security.declareProtected(PERMISSION_DELETE_OBJECTS,
                              'admin_deletions_html')

    def admin_deletions_html(self, REQUEST=None, RESPONSE=None):
        """ """
        return self.getFormsTool().getContent({'here': self},
                                              'site_admin_deletions')
    ...
    def dumpFormsLayout(self):
        ...
        if not os.path.isdir(path_forms):
            try:
                ...
                raise OSError('Can\'t create directory %s' % path_forms)
    ...
    def admin_properties(self, REQUEST=None, **kwargs):
        ...
        self.default_geographical_coverage = kwargs.get(
            'default_geographical_coverage', False)
    ...
    def admin_savemaintopic_image(self, mainsection, upload_picture_url,
        ...
        def_height = 75
        main_section_images = self._get_mainsection_images_folder()
        self.inherit_mainsection_image = 'inherit_mainsection_image' in REQUEST
        self._p_changed = True

        return self._admin_save_image(mainsection, upload_picture_url,
                                      x1, y1, x2, y2, width, height, def_width,
                                      def_height, main_section_images, REQUEST)

    security.declareProtected(PERMISSION_PUBLISH_OBJECTS,
                              'admin_save_slider_image')

    def admin_save_slider_image(self, sliderimage, upload_picture_url,
                                x1, y1, x2, y2, width, height, picture_title,
                                picture_subtitle, delete_picture=None,
                                REQUEST=None):
        """ """
        ...
        return self._admin_save_image(sliderimage, upload_picture_url,
                                      x1, y1, x2, y2, width, height, def_width,
                                      def_height, slider_images, REQUEST,
                                      picture_title, picture_subtitle)

    security.declareProtected(PERMISSION_PUBLISH_OBJECTS,
                              'admin_delete_slider_image')
...
def process_picture(picture, crop_coordinates, F_WIDTH, F_HEIGHT):
    ...
    if crop_coordinates[2] - crop_coordinates[0] == 0:
        ...
        if F_HEIGHT * x > F_WIDTH * y:
            crop_coordinates = ((x - F_WIDTH * y / F_HEIGHT) / 2, 0,
                                (x + F_WIDTH * y / F_HEIGHT) / 2, y)
        else:
            crop_coordinates = (0, (y - (F_HEIGHT * x / F_WIDTH)) / 2, x,
                                (y + (F_HEIGHT * x / F_WIDTH)) / 2)
...
def crop_image(ob, image_default_size):
    ...
    if (image_default_size[1] * x) > (image_default_size[0] * y):
        ...
        data = {
            ...
            'x2': x,
            'y2': (y + (image_default_size[1] * x
                   / image_default_size[0])) / 2,
            'title': title,
        }
