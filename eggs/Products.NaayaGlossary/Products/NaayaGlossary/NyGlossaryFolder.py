...
class NyGlossaryFolder(Folder, utils, glossary_export, catalog_utils):
    ...
    manage_addGlossaryElement_html = (
        NyGlossaryElement.manage_addGlossaryElement_html)
    ...
    def get_objects_sorted(self, lang_code):
        """ """
        l_elems = self.objectValues([NAAYAGLOSSARY_ELEMENT_METATYPE])
        return self.utSortObjsListByAttr(
            l_elems, self.get_language_by_code(lang_code), 0, lower=True)
    ...
    security.declareProtected(PERMISSION_MANAGE_NAAYAGLOSSARY,
                              'manage_folder_properties')

    def manage_folder_properties(self, title='', subjects=[], source='',
        ...
        self.approved = approved
        if REQUEST:
            REQUEST.RESPONSE.redirect('properties_html?save=ok')
    ...
    properties_html = NaayaPageTemplateFile(
        'zpt/NaayaGlossaryFolder/properties', globals(),
        'glossary_folder_properties')
    ...
    def manageNameTranslations(self, lang_code='', translation='',
        ...
        if REQUEST:
            return REQUEST.RESPONSE.redirect('translations_html')
    ...
    def getGlossElems(self):
        """ """
        return self.utSortObjsListByAttr(self.objectValues(
            [NAAYAGLOSSARY_ELEMENT_METATYPE]), 'id', 0)
