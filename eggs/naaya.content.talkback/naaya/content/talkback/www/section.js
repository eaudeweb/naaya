...
function is_ie9(){
    return (
        jQuery.browser.msie !== undefined &&
        jQuery.browser.version.slice(0,2) === "9."
    );
}

var inline_comments = $('ul.tb-comments-tree').length > 0;
...
function _closeForm(iframe, comment_box, resize_interval, overlay) {
    var tinymce = iframe[0].contentWindow.tinymce;
    var mce_text = tinymce === undefined ? '' : tinymce.editors[0].getContent();
...
function comment_add(evt) {
    ...
    var comment_box = $('<div ></div>').insertAfter($link)
        .addClass("talkback-comment_floating_box");
    ...
    iframe.load(function(){
        ...
        if(! inline_comments) {
            ...
            var infomsg = $(
                'div.message.information', this.contentDocument).text();
    ...
    function closeForm() {
        _closeForm(iframe, comment_box, resize_interval, overlay);
    }

    close_buttons.on('click', closeForm);
