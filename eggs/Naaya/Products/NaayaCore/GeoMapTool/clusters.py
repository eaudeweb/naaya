...
def getFullGridSize(size_y, size_x, lat_min, lat_max, lon_min, lon_max):
    ...
    return (
        int(math.floor(size_y * (360. / abs(lon_max - lon_min)))),
        int(math.floor(size_x * (360. / abs(lon_max - lon_min)))),
        # please note d(lon) not 100% of time constant - google maps
    )
...
def calc_centers(lat_min, lat_max, lon_min, lon_max, size_x,
                 size_y, points):
    ty_min, ty_max, tx_min, tx_max = tile_limits(size_y, size_x, lat_min,
...
def kmeans(lat_min, lat_max, lon_min, lon_max, points, size_x, size_y=None):
    """
    size_x, size_y - the dimensions of the initial grid of centers on full map
    """
    # default arguments for size_y and centers_step
    if size_y is None:
        size_y = size_x

    size_y, size_x = getFullGridSize(size_y, size_x, lat_min, lat_max, lon_min,
                                     lon_max)

    return calc_centers(lat_min, lat_max, lon_min, lon_max, size_x, size_y,
                        points)


def get_discretized_limits(lat_min, lat_max, lon_min, lon_max, size_x,
                           size_y=None):
    """
    ...
    size_y, size_x = getFullGridSize(size_y, size_x, lat_min, lat_max,
                                     lon_min, lon_max)
    ...
    tlat_min, tlon_min = getLatLonFromTile(size_y, size_x, ty_min, tx_min,
                                           0., 0.)
    tlat_max, tlon_max = getLatLonFromTile(size_y, size_x, ty_max, tx_max,
                                           0., 0.)
