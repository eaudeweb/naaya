""" Ported Unindex._apply_index from
    ...
    Note: In ZCatalog >= 5 (Zope 5), Products.PluginIndexes.common was
    removed and the UnIndex API changed. This patch is only applied on
    older ZCatalog versions.
"""
import logging
...
try:
    ...
    from BTrees.IIBTree import intersection, IISet, multiunion
    from Products.PluginIndexes.common.util import parseIndexRequest
    from Products.PluginIndexes.common.UnIndex import UnIndex
    _HAS_OLD_UNINDEX = True
except ImportError:
    _HAS_OLD_UNINDEX = False


def patch_zcatalog_unindex():
    if not _HAS_OLD_UNINDEX:
        log.info('Skipping UnIndex._apply_index patch (not needed for '
                 'this ZCatalog version)')
        return
