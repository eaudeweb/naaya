...
def addNyFolder(self, id='', REQUEST=None, contributor=None,
    ...
    # try:
    #     sortorder = abs(int(kwargs.get('sortorder')))
    # except:
    ...
    if form_errors:
        if REQUEST is None:
            ...
            import transaction
            # because we already called _create_NyZzz_object
            transaction.abort()
            ob._prepare_error_response(REQUEST, form_errors, schema_raw_data)
            REQUEST.RESPONSE.redirect(
                '%s/folder_add_html' % self.absolute_url())
            return
    ...
    if REQUEST is not None:
        ...
        if (referer == 'folder_manage_add' or
                referer.find('folder_manage_add') != -1):
...
def importNyFolder(
        self, param, id, attrs, content, properties, discussion, objects):
...
class NyFolder(
    ...
    def all_meta_types(self, interfaces=None):
        ...
        if len(folder_meta_types) > 0:
            # filter meta types
            l = list(filter(
                lambda x: x['name'] in folder_meta_types, Products.meta_types))
            # handle uninstalled pluggable meta_types
            pluggable_meta_types = self.get_pluggable_metatypes()
            pluggable_installed_meta_types = \
                self.get_pluggable_installed_meta_types()
            t = copy(l)
            for x in t:
    ...
    def export_this_tag_custom(self):
        return \
            'custom_index="%s" maintainer_email="%s" folder_meta_types="%s"' % \
        ...
        for i in self.getUploadedImages():
            ra('<img param="0" id="%s" content="%s" />' %
                (self.utXmlEncode(i.id()),
                 self.utXmlEncode(self.utBase64Encode(str(i.data)))))
        return ''.join(r)
    ...
    def checkPermissionManageObjects(self, sort_on='title', sort_order=0):
        ...
        folder_listing_info This function is called on the folder index and it
        checkes whether or not to display the various buttons on that form """
        ...
        # Naaya folders
        sorted_folders = self.utSortObjsListByAttr(
            self.getFolders(), sort_on, sort_order)
        for x in self.utSortObjsListByAttr(sorted_folders, 'sortorder', 0):
            ...
            if del_permission or copy_permission:
                btn_select = 1
            if del_permission and copy_permission:
                btn_cut = 1
            if del_permission:
                btn_delete = 1
            if copy_permission:
    ...
    def hasPendingContent(self):
        return len(self.getPendingContent()) > 0
    ...
    def getSortedObjects(self):
        return self.utSortObjsListByAttr(self.getObjects(), 'sortorder', 0)

    security.declareProtected(
        PERMISSION_PUBLISH_OBJECTS, 'isFeedbackCustomized')
    ...
    def getAdministratorsEmails(self, roles=['Administrator']):
        ...
        for k, v in self.get_local_roles():
            if len(self.utListIntersection(roles, v)):
                ra(k)
        return self.getAuthenticationTool().getUsersEmails(r)

    security.declareProtected(
        PERMISSION_PUBLISH_OBJECTS, 'getFeedbackCustomizedEmail')

    def getFeedbackCustomizedEmail(self):
        """
        Returns the email addresses.
        """
        buf = self.getSite().folder_customized_feedback.get(
            '/'.join(self.getPhysicalPath()), None)
        if isinstance(buf, tuple):
            return buf[0]

    security.declareProtected(
        PERMISSION_PUBLISH_OBJECTS, 'getFeedbackCustomizedPostal')
    ...
    def admin_folder_feedback_form(self, who=0, username='', email='',
        ...
        if err:
            ...
            self.getEmailTool().sendFeedbackEmail(
                l_to, username, email, comments)
        if REQUEST:
    ...
    def admin_folder_feedback(self, status='0', emails='', postal='',
        ...
        if REQUEST:
            self.setSessionInfoTrans(
                MESSAGE_SAVEDCHANGES, date=self.utGetTodayDate())
            REQUEST.RESPONSE.redirect(
                '%s/administration_feedback_html' % self.absolute_url())
    ...
    def admin_folder_revokeroles(self, roles=[], REQUEST=None):
        ...
        if REQUEST:
            ...
            REQUEST.RESPONSE.redirect(
                '%s/administration_users_html' % self.absolute_url())
    ...
    def admin_folder_portlets(self, portlets=[], folder='', mode='',
        ...
        if mode == 'delete':
            ...
            self.set_right_portlets_locations(
                folder, self.utConvertToList(portlets))
        if REQUEST:
            self.setSessionInfoTrans(
                MESSAGE_SAVEDCHANGES, date=self.utGetTodayDate())
            REQUEST.RESPONSE.redirect(
                '%s/administration_portlets_html' % self.absolute_url())
    ...
    def admin_folder_addroles(self, name='', roles=[], location='',
        ...
        if REQUEST:
            if err != '':
                self.setSessionErrorsTrans(err)
            if success:
                self.setSessionInfoTrans(MESSAGE_ROLEADDED, user=name)
            if not err:
    ...
    def delLogo(self):
        ...
        """
        try:
            self.manage_delObjects('logo.gif')
        except:
            pass
    ...
    def manageSubobjects(self, REQUEST=None, **kwargs):
        ...
        if kwargs.get('default', ''):
            ...
            new_types.extend(
                self.utConvertToList(kwargs.get('ny_subobjects', [])))
        ...
        if REQUEST:
            ...
            redirect = REQUEST.get(
                'redirect_url', 'manage_folder_subobjects_html')
            redirect += '?save=ok'
    ...
    def setSortOrder(self, ids, REQUEST):
        ...
        for id in self.utConvertToList(ids):
            try:
                sortorder = abs(int(REQUEST.get('%s__sortorder' % id, 0)))
            except:
                sortorder = DEFAULT_SORTORDER
            try:
                ob = self._getOb(id)
        ...
        if REQUEST:
            ...
            if success:
                self.setSessionInfoTrans(
                    MESSAGE_SAVEDCHANGES, date=self.utGetTodayDate())
            REQUEST.RESPONSE.redirect(
                '%s/basketofvalidation_html' % self.absolute_url())
            ...
            if self._getOb(ids[0]).meta_type in ['Naaya File', 'Naaya ExFile',
                ...
                try:
                    ...
                    self.setSessionErrorsTrans(
                        "Item %s could not be renamed." % ids[0])
    ...
    def _page_result(self, p_result, p_start):
        ...
        try:
            ...
            p_start = 0
        batch_obj = batch_utils(
            NUMBER_OF_RESULTS_PER_PAGE, len(p_result[6]), p_start)
        if len(p_result[6]):
    ...
    def getEntries(self, author='', tag='', p_start=0, howmany=-1):
        ...
        return self._page_result(
            self.checkPermissionManageEntries(author, howmany, tag), p_start)
    ...
    def entries_rdf(self, REQUEST=None, RESPONSE=None):
        ...
        ra('<description><![CDATA[%s]]></description>' %
           self.utXmlEncode(s.description))
        ra('<dc:identifier>%s</dc:identifier>' %
           self.utXmlEncode(s.portal_url))
        ra('<dc:date>%s</dc:date>' %
           self.utShowFullDateTimeHTML(self.utGetTodayDate()))
        ...
        for entry in entries:
            ra('<item rdf:about="%s">' %
               self.utXmlEncode(entry.absolute_url()))
            ra('<title>%s</title>' % self.utXmlEncode(entry.title))
            ra('<url>%s</url>' % self.utXmlEncode(entry.absolute_url()))
            ra('<description><![CDATA[%s]]></description>' %
               self.utXmlEncode(entry.content[:255]))
            ra('<dc:date>%s</dc:date>' %
               self.utShowFullDateTimeHTML(entry.releasedate))
            ra('<dc:subject>%s</dc:subject>' % self.utXmlEncode(entry.title))
            ra('<dc:creator>%s</dc:creator>' %
               self.utXmlEncode(entry.contributor))
            ra('</item>')
        ra("</rdf:RDF>")
    ...
    def comments_rdf(self, REQUEST):
        """ """
        catalog_tool = self.getCatalogTool()
        comments = catalog_tool.getLatestComments(
            self.getPhysicalPath(), limit=20)
    ...
    security.declareProtected(
        view_management_screens, 'folder_manage_edit_html')
    folder_manage_edit_html = PageTemplateFile(
        'zpt/folder_manage_edit', globals())

    security.declareProtected(view_management_screens,
                              'manage_folder_subobjects_html')
    ...
    def standard_html_header(self, REQUEST=None, RESPONSE=None, **args):
        ...
        if pt is None:
            ...
            return pt.pt_render(extra_context=args).split(
    ...
    def standard_html_footer(self, REQUEST=None, RESPONSE=None):
        ...
        if pt is None:
            ...
            pt_context['skin_files_path'] = \
                self.getLayoutTool().getSkinFilesPath()
            return pt.pt_render(extra_context=pt_context).split(
    ...
    def index_html(self, REQUEST=None, RESPONSE=None):
        ...
        if tmpl is None:
            ...
            tmpl = self.getFormsTool()[
                self.default_form_id].aq_base.__of__(self)
    ...
    def index_atom(self, REQUEST=None, RESPONSE=None):
        ...
        return self.getSyndicationTool().syndicateAtom(
            context=self,
            items=self.getPublishedContent(),
            lang=lang,
            REQUEST=REQUEST)
    ...
    def switchToLanguage(self, REQUEST=None, **kwargs):
        ...
        if hasattr(doc, 'switch_content_to_language'):
            ...
            if doc.meta_type in ('Naaya Extended File',):
                ...
                files = dict([(key, value)
                              for key, value in files.items() if key != lang])
    ...
    def updateSessionFrom(self, REQUEST=None, **kwargs):
        ...
        # Return
        REQUEST.RESPONSE.redirect(
            '%s/edit_html?lang=%s' % (doc.absolute_url(), lang))
    ...
    def basketofapprovals_html(self, REQUEST=None, RESPONSE=None):
        """ """
        return self.getFormsTool().getContent(
            {'here': self}, 'folder_basketofapprovals')

    security.declareProtected(
        PERMISSION_VALIDATE_OBJECTS, 'basketofvalidation_html')

    def basketofvalidation_html(self, REQUEST=None, RESPONSE=None):
        """ """
        return self.getFormsTool().getContent(
            {'here': self}, 'folder_basketofvalidation')
    ...
    def menusubmissions(self, REQUEST=None, RESPONSE=None):
        """ """
        return self.getFormsTool().getContent(
            {'here': self}, 'folder_menusubmissions')
    ...
    security.declareProtected(
        PERMISSION_PUBLISH_OBJECTS, 'administration_basket_html')

    def administration_basket_html(self, REQUEST=None, RESPONSE=None):
        """ """
        return self.getFormsTool().getContent(
            {'here': self}, 'folder_administration_basket')

    security.declareProtected(
        PERMISSION_PUBLISH_OBJECTS, 'administration_logo_html')

    def administration_logo_html(self, REQUEST=None, RESPONSE=None):
        """ """
        return self.getFormsTool().getContent(
            {'here': self}, 'folder_administration_logo')

    security.declareProtected(
        PERMISSION_PUBLISH_OBJECTS, 'administration_users_html')

    def administration_users_html(self, REQUEST=None, RESPONSE=None):
        """ """
        return self.getFormsTool().getContent(
            {'here': self}, 'folder_administration_users')

    security.declareProtected(
        PERMISSION_PUBLISH_OBJECTS, 'administration_source_html')

    def administration_source_html(self, REQUEST=None, RESPONSE=None):
        """ """
        return self.getFormsTool().getContent(
            {'here': self}, 'folder_administration_source')

    security.declareProtected(
        PERMISSION_PUBLISH_OBJECTS, 'administration_portlets_html')

    def administration_portlets_html(self, REQUEST=None, RESPONSE=None):
        """ """
        return self.getFormsTool().getContent(
            {'here': self}, 'folder_administration_portlets')

    security.declareProtected(
        PERMISSION_PUBLISH_OBJECTS, 'administration_feedback_html')

    def administration_feedback_html(self, REQUEST=None, RESPONSE=None):
        """ """
        return self.getFormsTool().getContent(
            {'here': self}, 'folder_administration_feedback')
