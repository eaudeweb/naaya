...
BOOL CInstallPage::CopyNecessaryFiles(CStringArray& arrLog)
{
	...
	// move PTKUninstall.exe from bin to application path
	CString strFilePath, strBuffer;
	strFilePath.Format("%s\\PTKUninstall.exe", pWizardData->m_strPath + strBinFolder);
	strBuffer.Format(CRString(IDS_MOVE_FILE), strFilePath, pWizardData->m_strPath + "\\PTKUninstall.exe");
    arrLog.Add(strBuffer);
	if (toolz.InternalOverwriteFile(pWizardData->m_strPath + strBinFolder + "\\PTKUninstall.exe", pWizardData->m_strPath + "\\PTKUninstall.exe"))
	{
		strError.Format(CRString(IDS_FAILED_MOVE_FILE), pWizardData->m_strPath + strBinFolder + "\\PTKUninstall.exe", pWizardData->m_strPath + "\\PTKUninstall.exe");
		arrLog.Add(strError);
		return FALSE;
	}
	else
		toolz.InternalDeleteFile(pWizardData->m_strPath + strBinFolder + "\\PTKUninstall.exe");
...
BOOL CInstallPage::CreateShortcuts(CStringArray& arrLog)
{
	...
	strBuffer.Format("%s\\PTKConsole.exe", pWizardData->m_strPath + strBinFolder);
	...
	strBuffer.Format("%s\\PTKUninstall.exe", pWizardData->m_strPath);
