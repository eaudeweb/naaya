...
__version__=''' $Id: paragraph.py 2456 2004-10-19 17:52:20Z rgbecker $ '''
    ...
    for n in range(len(words), 0, -1):
        ...
        if hasattr(f,'cbDefn'):
            ...
            if tx.XtraState.rise!=f.rise:
                ...
                tx.setRise(f.rise)
            tx._textOut(f.text,f is words[0])  # cheap textOut

def _putFragLine(tx,words):
    cur_x = 0
    if hasattr(tx, 'isRTL') and tx.isRTL():
        return _putFragLineRTL(tx,words)
    for f in words:
        if hasattr(f,'cbDefn'):
            func = getattr(tx._canvas,f.cbDefn.name,None)
            if not func:
