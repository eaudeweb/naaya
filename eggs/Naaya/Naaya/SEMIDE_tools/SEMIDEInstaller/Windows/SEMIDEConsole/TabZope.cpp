...
BOOL CTabZope::OnInitDialog() 
{
	...
	CString strApplicationFolderPath, strZopeConfPath;

	toolz.CTReadRegistryKey(strRegString, CRString(IDS_REGAPPLICATIONPATH), strApplicationFolderPath);
	strZopeConfPath = strApplicationFolderPath + CRString(IDS_FOLDER_ZOPE) + "\\etc\\zope.conf";

	GetDlgItem(IDC_ZOPECONF_PATH)->SetWindowText(strZopeConfPath);
