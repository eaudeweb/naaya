...
/* Class: com.finsiel.gis.gml.fGmlDrawPoint

   Draw a group of points into a specific container

   Coordinate transformation will be applied from point coordinates to container coordinates
...
class com.finsiel.gis.gml.fGmlDrawPoint {
	...
	*/
	public function fGmlDrawPoint(d_array:Array, d_container:MovieClip, myMB, _s_arr:Array) {
	...
	public function begin_draw() {
		var my_mapex = d_map.getMapextend();
	...
	private function select_draw(i) {
		var xp:Number = new Number(d_arr[i].x);
		var yp:Number = new Number(d_arr[i].y);
		if (xp<=this.maxx && xp>=this.minx && yp<=this.maxy && yp>=this.miny) {
	...
	private function draw_now(p:Object, id:Number) {
		...
		var ds = new fGmlStylePoint(d_map,_myMB)
