...
def addNyReportSection(self, id='', title='', description='', coverage='', keywords='',
    ...
    if not(l_referer == 'reportsection_manage_add' or l_referer.find('reportsection_manage_add') != -1) and REQUEST:
    ...
    if not len(r):
        ...
        if REQUEST is not None:
            if l_referer == 'reportsection_manage_add' or l_referer.find('reportsection_manage_add') != -1:
...
class NyReportSection(NyAttributes, reportsection_item, NyContainer, NyEpozToolbox, NyCheckControl):
    ...
    def getSections(self): return self.objectValues(METATYPE_OBJECT)
