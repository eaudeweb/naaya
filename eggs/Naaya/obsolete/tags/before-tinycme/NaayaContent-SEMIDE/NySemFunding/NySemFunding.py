...
from semfunding_item                    import semfunding_item
...
def semfunding_add_html(self, REQUEST=None, RESPONSE=None):
    """ """
    return self.getFormsTool().getContent({'here': self, 'kind': METATYPE_OBJECT, 'action': 'addNySemFunding'}, 'semfunding_add')

def addNySemFunding(self, id='', title='', description='', coverage='', keywords='', sortorder='',
    ...
    """
    Create a Funding type of object.
    """
    #process parameters
    ...
    if not(l_referer == 'manage_addNySemFunding_html' or l_referer.find('manage_addNySemFunding_html') != -1) and REQUEST:
        r = self.getSite().check_pluggable_item_properties(METATYPE_OBJECT, id=id, title=title, \
            description=description, coverage=coverage, keywords=keywords, sortorder=sortorder, \
            releasedate=releasedate, discussion=discussion, \
            funding_source=funding_source, funding_programme=funding_programme, funding_type=funding_type, \
    ...
    if not len(r):
        ...
        if REQUEST is not None:
            if l_referer == 'manage_addNySemFunding_html' or l_referer.find('manage_addNySemFunding_html') != -1:
        ...
        if REQUEST is not None:
            ...
            self.set_pluggable_item_session(METATYPE_OBJECT, id=id, title=title, \
                ...
                releasedate=releasedate, discussion=discussion, \
                funding_source=funding_source, funding_programme=funding_programme, funding_type=funding_type, \
                funding_rate=funding_rate, lang=lang)
...
def importNySemFunding(self, param, id, attrs, content, properties, discussion, objects):
    ...
    if param == 3:
        ...
        if param in [0, 1] or (param==2 and ob is None):
            ...
            addNySemFunding(self, id=id,
                sortorder=attrs['sortorder'].encode('utf-8'),
                funding_type=attrs['funding_type'].encode('utf-8'),
                funding_rate=attrs['funding_rate'].encode('utf-8'),
...
class NySemFunding(NyAttributes, semfunding_item, NyItem, NyCheckControl):
    """ """
    ...
    def __init__(self, id, title, description, coverage, keywords, sortorder,
        ...
        """ """
        self.id = id
        semfunding_item.__dict__['__init__'](self, title, description, coverage,
            keywords, sortorder, funding_source, funding_programme, funding_type,
    ...
    def manageProperties(self, title='', description='', coverage='', keywords='',
        sortorder='', approved='', funding_source='', funding_programme='',
        funding_type='', funding_rate='',
    ...
    def commitVersion(self, REQUEST=None):
        ...
        self.funding_source =       self.version.funding_source
        self.funding_programme =    self.version.funding_programme
        self.funding_type =         self.version.funding_type
        self.funding_rate =         self.version.funding_rate
    ...
    def startVersion(self, REQUEST=None):
        ...
        self.version = semfunding_item(self.title, self.description,
            self.coverage, self.keywords, self.sortorder, self.funding_source,
            self.funding_programme, self.funding_type, self.funding_rate,
    ...
    def saveProperties(self, title='', description='', coverage='', keywords='',
        sortorder='', funding_source='',
        funding_programme='', funding_type='', funding_rate='',
        ...
        if not len(r):
            ...
            if REQUEST is not None:
                ...
                self.set_pluggable_item_session(METATYPE_OBJECT, id=id, title=title, \
                    description=description, coverage=coverage, keywords=keywords, sortorder=sortorder, \
                    releasedate=releasedate, discussion=discussion, \
                    funding_source=funding_source, funding_programme=funding_programme, \
                    funding_type=funding_type, funding_rate=funding_rate)
                REQUEST.RESPONSE.redirect('%s/edit_html?lang=%s' % (self.absolute_url(), lang))
            else:
                raise Exception, '%s' % ', '.join(r)
