...
def _check_ffmpeg():
    ...
    if process.returncode!=1:
        raise MediaConverterError('could not run ffmpeg: "ffmpeg -h" has exited with code %s' % (process.returncode, ))
    if "--enable-libmp3lame" not in stdout:
        raise MediaConverterError('ffmpeg was not compiled with --enable-libmp3lame')
...
def _check_flvtool2():
    ...
    if process.returncode:
        raise MediaConverterError('could not run flvtool2: "flvtool2 -H" has exited with code %s' % (process.returncode, ))
