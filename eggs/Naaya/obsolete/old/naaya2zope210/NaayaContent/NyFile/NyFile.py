...
def _create_NyFile_object(parent, id, title, file, precondition, contributor):
    ...
    while parent._getOb(id, None):
        i += 1
        id = '%s-%u' % (id, i)
    ob = NyFile(id, title, file, precondition, contributor)
...
class NyFile(file_item, NyAttributes, NyItem, NyFolderishVersioning, NyCheckControl, NyValidation, NyContentType):
    """ """
