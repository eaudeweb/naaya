...
class SchemaAdminTest(NaayaFunctionalTestCase.NaayaFunctionalTestCase):
    ...
    def test_schemas_list(self):
        ...
        self.failUnless('portal_schemas/document_item/admin_html' in html)

    def test_schema(self):
        self.browser.go('http://localhost/portal/portal_schemas/document_item/admin_html')

        html = self.browser.get_html()
        self.failUnless('<h1>Manage content types - HTML Document</h1>' in html)

        self.failUnless('Title' in html)
        self.failUnless('portal_schemas/document_item/title-property/admin_html' in html)
        self.failUnless('Release date' in html)
        self.failUnless('portal_schemas/document_item/releasedate-property/admin_html' in html)

    def test_schema_widget(self):
        self.browser.go('http://localhost/portal/portal_schemas/document_item/title-property/admin_html')
