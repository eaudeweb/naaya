...
from Products.SEMIDE.managers.semide_zip import *
from Products.Localizer.LocalPropertyManager import LocalPropertyManager, LocalProperty
import NyPhoto

manage_addNyPhotoFolder_html = PageTemplateFile('zpt/photofolder_manage_add', globals())
def manage_addNyPhotoFolder(self, id='', title='', quality='', sortorder='', discussion='',
    ...
    #create object
    ob = NyPhotoFolder(id, title, quality, sortorder, approved, approved_by, releasedate, lang)
...
class NyPhotoFolder(NyAttributes, LocalPropertyManager, NyContainer):
    ...
    icon = 'misc_/SEMIDEPhotoArchive/NyPhotoFolder.gif'
    ...
    def __init__(self, id, title, quality, sortorder, approved, approved_by, releasedate, lang):
        """ """
        self.id = id
    ...
    def getPublishedIds(self): return [x.id for x in self.getObjects() if x.approved==1 and x.submitted==1]
    ...
    def get_all_photos(self):
        """ sends a string like: id1/id2/id3..."""
        allphotos_string = ""
        for i in self.getPublishedIds():
            allphotos_string = allphotos_string + i + "/"
        return allphotos_string
    ...
    def manageProperties(self, title='', quality='', sortorder='', approved='', releasedate='',
    ...
    def saveProperties(self, title='', quality='', sortorder='', discussion='', lang=None,
        REQUEST=None):
        """ """
        try: quality = abs(int(quality))
        except: quality = DEFAULT_QUALITY
    ...
    security.declarePublic('downloadObjects')
    def downloadObjects(self, ids=None, REQUEST=None):
        """ """
        ids = self.utConvertToList(ids)
        download_ids = ""
        fld_url = self.absolute_url(1)
        zip_file_name=self.id + '.zip'
        if REQUEST.has_key('all'):
            download_ids = self.get_all_photos()
        else:
            for i in ids:
                download_ids = download_ids + i + "/"
        return self.zip_generator(fld_url=fld_url, zip_file_name=zip_file_name, download_ids=download_ids, RESPONSE=REQUEST.RESPONSE)
    ...
    security.declareProtected(PERMISSION_ADD_PHOTO, 'uploadZipFile')
    def uploadZipFile(self, file='', REQUEST=None):
        """
        Expand a zipfile into a number of Photos.
        Go through the zipfile and for each file create a Naaya Photo object.
        """
        err = ''
        try:
