...
OBJECT_CONSTRUCTORS = ['manage_addNyPublication_html', 'publication_add_html',
                       'addNyPublication', 'importNyPublication']
OBJECT_ADD_FORM = 'publication_add_html'
DESCRIPTION_OBJECT = ('This is Naaya Publication type. '
                      'It can be used for published articles or books.')
PREFIX_OBJECT = 'pub'
PROPERTIES_OBJECT = {
    'id':           (0, '', ''),
    ...
    'discussion':   (0, '', ''),
    'locator':      (1, MUST_BE_NONEMPTY,
                     'The Publication field must have a value.'),
    'lang':         (0, '', '')
}
DEFAULT_SCHEMA = {
    'original_title':   dict(sortorder=11, widget_type='String',
                             label='Original title'),
    'author':           dict(sortorder=12, widget_type='String',
                             label='Author'),
    'publisher':        dict(sortorder=13, widget_type='String',
                             label='Publisher'),
    'editor':           dict(sortorder=14, widget_type='String',
                             label='Editor'),
    'publication_year': dict(sortorder=15, widget_type='String',
                             data_type='int', label='Publication year'),
    'volume':           dict(sortorder=16, widget_type='String',
                             label='Volume'),
    'issue_no':         dict(sortorder=17, widget_type='String',
                             label='Issue No.'),
    'issue_date':       dict(sortorder=18, widget_type='Date',
                             data_type='date', label='Issue date'),
    'description':      dict(sortorder=20, widget_type='TextArea',
                             label='Short description of content',
                             localized=True, tinymce=True),
    'pages':            dict(sortorder=80, widget_type='String',
                             data_type='int', label='Page(s)'),
    'document_type':    dict(sortorder=90, widget_type='String',
                             label='Document type'),
    'isbn':             dict(sortorder=100, widget_type='String',
                             label='ISBN'),
    'languages':        dict(sortorder=110, widget_type='String',
                             label='Language(s)'),
    'locator':          dict(sortorder=120, widget_type='String',
                             label='Internet access (URL)', localized=True),
}
...
config = {
    ...
    'description': ('This is Naaya Publication type. '
                    'It can be used for published articles or books.'),
    ...
    '_misc': {
        ...
        'NyPublication_marked.gif': ImageFile('www/NyPublication_marked.gif',
                                              globals()),
        },
    }
...
def publication_add_html(self, REQUEST=None, RESPONSE=None):
    ...
    return self.getFormsTool().getContent(
        {'here': self, 'kind': METATYPE_OBJECT, 'action': 'addNyPublication',
         'form_helper': form_helper}, 'publication_add')
...
def addNyPublication(self, id='', REQUEST=None, contributor=None, **kwargs):
    ...
    id = self.utSlugify(id or schema_raw_data.get('title', '') or
                        PREFIX_OBJECT)
    ...
    if REQUEST is not None:
        ...
        if (l_referer == 'publication_manage_add' or
                l_referer.find('publication_manage_add') != -1):
...
def importNyPublication(self, param, id, attrs, content, properties,
                        discussion, objects):
    ...
    if param == 3:
        ...
        if param in [0, 1] or (param == 2 and ob is None):
            ...
            ob = _create_NyPublication_object(
                self, id, id, '',
            ...
            ob.checkThis(
                attrs['validation_status'].encode('utf-8'),
...
class NyPublication(publication_item, NyAttributes, NyItem, NyCheckControl,
    ...
    def saveUpload(self, source='file', file='', url='', lang=None,
                   REQUEST=None):
        """ """
    ...
    manage_edit_html = PageTemplateFile('zpt/publication_manage_edit',
                                        globals())
