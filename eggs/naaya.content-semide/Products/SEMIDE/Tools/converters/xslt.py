...
class TmpFile:
    ...
    def __init__(self, data):
        ...
        try:
            ...
            open(self.fname,'w+b').write(data.encode('utf-8'))
...
class Converter:
    ...
    def convert(self, doc, style):
        ...
        if sys.platform == 'win32':
            html = self.execute('xsltproc.exe "%s" "%s"' % (style_name, xml_name))
        else:
            html = self.execute('xsltproc "%s" "%s" 2> /dev/null' % (style_name, xml_name))
        return html
